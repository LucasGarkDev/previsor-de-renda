"""
Treinamento CatBoost V5 — Espírito Santo
Dataset: v3
"""
...
def train_catboost_v5_es():

    logger.info("Iniciando treino CatBoost V5 — ES")

    train_df = read_parquet(DATA_PROCESSED_DIR / "v3" / "train.parquet")
    val_df = read_parquet(DATA_PROCESSED_DIR / "v3" / "validation.parquet")

    train_df = train_df[train_df[UF_COLUMN] == UF_FOCUS]
    val_df = val_df[val_df[UF_COLUMN] == UF_FOCUS]

    logger.info(f"Train ES: {len(train_df)}")
    logger.info(f"Val ES: {len(val_df)}")
    ...
    logger.info(f"[VAL V5 ES] R²: {r2:.4f}")
    ...
    joblib.dump(bundle, "models/catboost_v5_es.joblib")

    return bundle


if __name__ == "__main__":
    train_catboost_v5_es()