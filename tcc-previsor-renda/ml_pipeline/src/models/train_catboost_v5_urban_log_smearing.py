"""
Treinamento CatBoost V5 — Urbano
Dataset: v3
"""
...
def train_catboost_v5_urban():

    logger.info("Iniciando treino CatBoost V5 — URBANO")

    train_df = read_parquet(DATA_PROCESSED_DIR / "v3" / "train.parquet")
    val_df = read_parquet(DATA_PROCESSED_DIR / "v3" / "validation.parquet")

    train_df = train_df[train_df[URBAN_COLUMN].astype(str) == "1"]
    val_df = val_df[val_df[URBAN_COLUMN].astype(str) == "1"]

    logger.info(f"Train urbano: {len(train_df)}")
    logger.info(f"Val urbano: {len(val_df)}")
    ...
    joblib.dump(bundle, "models/catboost_v5_urban.joblib")

    return bundle


if __name__ == "__main__":
    train_catboost_v5_urban()